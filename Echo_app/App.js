...
export default function App() {
  ...
  const setupSocket = (currentUser) => {
    ...
    socket.on("router_claimed", (data) =>
      setRouterUser(data.router_user || null)
    );
    ...
    socket.on("open_song", async (data) => {
      ...
      console.log(
        `[ROUTER - RECEBIDO] Evento "open_song" recebido. ID: ${song_id}`
      );
  ...
  async function doLogin() {
    ...
    try {
      ...
      if (error.response) {
        ...
        Alert.alert(
          "Erro de Conexão",
          `Não foi possível conectar ao servidor em ${SERVER_URL}. Verifique o IP, a rede Wi-Fi e o Firewall.`
        );
      } else {
        Alert.alert(
          "Erro Inesperado",
        ...
        <View style={styles.contentWrapper}>
          <View style={styles.card}>
            ...
            <Text style={styles.label}>
              Digite o endereço IP do computador onde o servidor está rodando.
            </Text>
